...
- To select one among many options presented in a dropdown: Use a `<select>` + `appearance: base-select` + `::picker(select)`. For more info see `branded-select-styling` (via `npx -p modern-web-guidance@latest -- modern-web retrieve "branded-select-styling"`)
